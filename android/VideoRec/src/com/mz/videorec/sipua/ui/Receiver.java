...
import com.mz.videorec.sipua.phone.Connection;
...
public class Receiver extends BroadcastReceiver {
				...
				if ((pstn_state == null || pstn_state.equals("IDLE"))
					...
					if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {
						String sUriSipRingtone = PreferenceManager
								.getDefaultSharedPreferences(mContext)
								.getString(
										com.mz.videorec.sipua.ui.Settings.PREF_SIPRINGTONE,
	...
	public static void onText(int type, String text, int mInCallResId, long base) {
		...
		if (type >= REGISTER_NOTIFICATION
				...
				&& !PreferenceManager.getDefaultSharedPreferences(
						...
						com.mz.videorec.sipua.ui.Settings.PREF_REGISTRATION,
						com.mz.videorec.sipua.ui.Settings.DEFAULT_REGISTRATION))
			text = null;
		...
		if (text != null) {
			...
			if (type == MISSED_CALL_NOTIFICATION) {
				...
				if (PreferenceManager.getDefaultSharedPreferences(
						Receiver.mContext).getBoolean(
						com.mz.videorec.sipua.ui.Settings.PREF_NOTIFY,
						com.mz.videorec.sipua.ui.Settings.DEFAULT_NOTIFY)) {
		...
		if (enable) {
			if (call_state == UserAgent.UA_STATE_IDLE
					...
					&& PreferenceManager
							.getDefaultSharedPreferences(mContext)
							.getString(
									com.mz.videorec.sipua.ui.Settings.PREF_POSURL,
									com.mz.videorec.sipua.ui.Settings.DEFAULT_POSURL)
							.length() > 0) {
				Location last = lm
	...
	static void enable_wifi(boolean enable) {
		if (!PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.mz.videorec.sipua.ui.Settings.PREF_OWNWIFI,
						com.mz.videorec.sipua.ui.Settings.DEFAULT_OWNWIFI))
			return;
		if (enable
				&& !PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getBoolean(
								com.mz.videorec.sipua.ui.Settings.PREF_WIFI_DISABLED,
								com.mz.videorec.sipua.ui.Settings.DEFAULT_WIFI_DISABLED))
			return;
		...
		edit.putBoolean(com.mz.videorec.sipua.ui.Settings.PREF_WIFI_DISABLED,
				!enable);
		edit.commit();
		if (enable) {
	...
	public static boolean isFast() {
		...
		if (PreferenceManager
				...
				|| PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getString(
								com.mz.videorec.sipua.ui.Settings.PREF_SERVER
										+ (""), "").equals(""))
			return false;
		if (wi != null) {
			if (!Sipdroid.release)
		...
		if (intentAction.equals(Intent.ACTION_BOOT_COMPLETED)) {
			...
			if (SystemClock.uptimeMillis() > lastscan + 45000
					&& PreferenceManager
							.getDefaultSharedPreferences(mContext)
							.getBoolean(
									com.mz.videorec.sipua.ui.Settings.PREF_SELECTWIFI,
									com.mz.videorec.sipua.ui.Settings.DEFAULT_SELECTWIFI)) {
				WifiManager wm = (WifiManager) mContext
						.getSystemService(Context.WIFI_SERVICE);
				WifiInfo wi = wm.getConnectionInfo();
